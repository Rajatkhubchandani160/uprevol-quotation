...
const Header = () => {
  ...
  <div className="navbar-start">
    <Link to={'/'}><img src='https://imgs.search.brave.com/NNk1t_KAInIjf_vl4BvTsEOBaSVjja1LZ3_WcvUHbMM/rs:fit:500:0:0/g:ce/aHR0cHM6Ly9icmFu/ZGxvZ29zLm5ldC93/cC1jb250ZW50L3Vw/bG9hZHMvMjAxNi8w/MS94aWFvbWktbG9n/b19icmFuZGxvZ29z/Lm5ldF9kc2Rodi01/MTJ4NTEyLnBuZw' width={43} className='rounded-xl mx-6'/></Link>
  ...
  <div className="navbar-center ml-9 hidden lg:flex">
   <input className='px-3 pr-40 p-1 rounded-l-full w-[10-vw] outline-none focus-within:shadow-md pl-4' type='text' placeholder='Search Product Here...' onChange={handleSearch}></input>
  ...
  <div className="navbar-end flex items-center">
    ...
    <div className="dropdown dropdown-end sm:block">
            <div tabIndex={0} role="button" className="mt-1">
              <div className='border-2 border-black mb-1 rounded-full w-8 mt-2 h-8 flex items-center justify-center text-xl'>
                {(!user || !user.user) ? <FaUserLarge size={17} /> : user.user.name[0]}
              </div>
            </div>
            {user && user.user ? (
              <div tabIndex={0} className="mt-4 z-[1] card card-compact dropdown-content w-52 bg-base-100 shadow">
    ...
    <div className="dropdown sm:hidden dropdown-end">
      <div tabIndex={0} role="button" className="mt-1">
        <div className='border-2 border-black mb-1 rounded-[50%] w-8 mt-2 h-8 flex items-center justify-center text-xl'>
        {(!user) || (!user.user) ? <FaUserLarge size={17}/>:user.user.name[0]}
        </div>
      </div>
      {(!user) || (!user.user)?null:
      <div tabInd ex={0} className="mt-4 z-[1] card card-compact dropdown-content w-52 bg-base-100 shadow">
        <div className="card-body">
           <span className="font-bold text-lg">
          {(!user) ||(!user.user) ?"Username":
          user.user.name}
          </span>
          {/* <span className="text-info text-sm break-all">
          {(!user) ||(!user.user) ?"Email":
          user.user.email}
          </span> */} 
          
          <Link to={'/profile/all-products'}><div className="card-actions">
            <button className="btn btn-primary btn-block">View Profile</button>
          </div></Link>
        </div>
      </div>}
    ...
    {(!user) || (!user.user)? 
    <Link to={"/Login"} className="btn mx-6 px-6 bg-red-600 
    hover:bg-red-700 text-white rounded-full">Login</Link>:
    <Link onClick={Handlelogout}  className="btn mx-6 px-6 bg-red-600 
    hover:bg-red-700 text-white rounded-full">Logout</Link>}
  </div>
</div>
   </>
  )
